...
**Status key:** ✅ Implemented | ⚠️ Partial | ❌ Missing | 🏗️ Contract only

## Layer 1 — Hearing (speech → signal)

| # | Capability | Model class | Buy vs Build | Wave | Status |
|---|---|---|---|---|---|
| 1 | *ASR (transcription)* | Whisper / WhisperX-class, later fine-tuned on Indian-English audio (Telugu/Hindi/Tamil L1) | Vendor/off-shelf first → fine-tune W3 | W1→W3 | ✅ Tier 1: faster-whisper |
| 2 | *Voice activity detection* | Silero-VAD class | Off-shelf, tiny | W1 | ✅ Tier 0: energy + Tier 1: Silero |
| 3 | *Forced alignment* | WhisperX / MFA (word & phoneme timestamps) | Off-shelf | W1 | ✅ Embedded in Pronunciation (wav2vec2 CTC) |
...
| # | Capability | Model class | Buy vs Build | Wave | Status |
|---|---|---|---|---|---|
| 4 | *Pronunciation scoring* | GOP (Goodness of Pronunciation) on wav2vec2/HuBERT embeddings, fine-tuned SpeechOcean762-style | Vendor API W1 → in-house W3 (shadow mode, per SRS E17) | W1→W3 | ✅ Tier 1: wav2vec2 GOP (char-level) |
| 5 | *Fluency & prosody scoring* | Feature-based regressor (gradient boosting on pause/rate/pitch features) — not a neural end-to-end model | Build (cheap, interpretable) | W1–2 | ⚠️ Tier 0 only: feature-based. No pitch/prosody model. |
| 6 | *Disfluency/filler detection* | Token classification on transcript + acoustic cues ("um," repetitions, self-corrections) | Build (small model) | W1–2 | ✅ Tier 1: transcript + pause rules |
| 7 | *Intelligibility model* ⭐ | Regressor predicting human intelligibility ratings, trained per L1 group on your own labelled panel data | *Build — this is your moat model.* Nobody has Indian-L1 intelligibility labels at your scale | W2–3 | ❌ Missing. Contract only. |
| 8 | *L1/accent identification* | Small classifier on speech embeddings (ECAPA-class) | Build later — routes L1-specific feedback and powers L1-difficulty stats | W3 | ❌ Missing. Contract only. |
...
| # | Capability | Model class | Buy vs Build | Wave | Status |
|---|---|---|---|---|---|
| 9 | *Grammar error detection* | GEC model (T5/LLM-based) on transcripts, error-typed | Off-shelf + tune | W1–2 | ⚠️ Rule-based (40 regexes), not ML. Indian English excluded. |
| 10 | *Content relevance & retell recall* | Sentence embeddings for key-point coverage + rubric-constrained LLM grading as one signal (never sole judge, per SRS) | Build thin layer on APIs | W1–2 | ✅ Word-overlap coverage. No embeddings/LLM. |

## Layer 4 — The intelligence nobody sees (this is where "really intelligent" lives)

| # | Capability | Model class | Buy vs Build | Wave | Status |
|---|---|---|---|---|---|
| 11 | *Item calibration* | IRT (2-parameter logistic): difficulty + discrimination per item, L1-split | Build — classical psychometrics, your speech-ML consultant + a psychometrics reference implement this | W2 (E14) | ❌ Missing. `app/engine/psychometrics/irt.py` exists but not wired. |
| 12 | *Skill mastery tracking* | Bayesian Knowledge Tracing first; Deep Knowledge Tracing only if data volume justifies | Build — drives the drill loop's "next weakness" | W2 | ❌ Missing. `SkillMastery` document exists but no BKT engine. |
| 13 | *Adaptive item selection* | IRT-information-based selection (computer-adaptive testing), later contextual bandits | Build on top of #11 | W2–3 | ❌ Missing. Random sampling with filters only. |
| 14 | *Crosswalk readiness prediction* | Gradient-boosted trees / regularized logistic on the feature record → outcome band, with isotonic/Platt calibration for honest confidence intervals | Build — gated per E15 (≥300 paired records, ≥75% band accuracy) | W2–3 | ❌ Missing. `CohortReadiness` is descriptive, not predictive. |
...
## Other Contracts (defined in `app/engine/contracts/`)

| Capability | Contract | Implementation | Notes |
|---|---|---|---|
| **TTS** | `language.py:42` `TTSProvider` | ❌ Missing | Prompt playback uses browser `speechSynthesis` fallback |
| **STORAGE** | `__init__.py:46` `StorageProvider` / `app/storage/base.py` | ❌ Missing | Local filesystem used directly via `app/storage/` |
| **NOTIFICATION** | `services.py:34` `NotificationProvider` | ❌ Missing | Engagement events logged but not sent |
| **PAYMENT** | `services.py:48` `PaymentProvider` | ❌ Missing | Invoice generation exists but no payment gateway |
...
- Per-tenant config with global defaults
