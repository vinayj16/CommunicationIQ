...
**Status Key:** ✅ Done · ⚠️ Partial · 🏗️ Contract Only · ❌ Not Started
...
- 🏗️ **PLAT-09** — Shadow-mode evaluation: run a new provider silently, compare against production before promotion. *(P2)* — Contract exists, Tier 1 fallback implemented
- 🏗️ **PLAT-10** — Canary routing: split live traffic by percentage across two providers for the same capability. *(P2)* — Contract exists
...
- ✅ **PLAT-17** — Question bank management: add/edit/delete questions for all 6 categories (reading, writing, listening, speaking, grammar, vocabulary). *(P1)*
- ✅ **PLAT-18** — Company-specific questions: TCS, Infosys, Wipro, Accenture, Cognizant content in question bank. *(P1)*
- ❌ **PLAT-15** — Internal support/ticketing queue with SLA tracking. *(P2)*
- ✅ **PLAT-16** — Role-scoped internal staff accounts (Super Admin, Tenant Admin, Student) with mandatory MFA. *(P1)* — MFA not yet implemented
...
- ✅ **TEN-03** — Bulk import students/staff via CSV/Excel, with validation and error reporting. *(P1)* — SSO directory sync not yet
...
- ⚠️ **TEN-08** — Drill-down from cohort dashboard into individual student diagnostic reports. *(P2)* — Basic view exists, not full diagnostic
...
- ✅ **STU-01** — Registration via tenant invite, tenant-permitted self-signup, or SSO. *(P1)* — SSO not yet
...
- ✅ **STU-04** — Configurable task types: Read Aloud, Repeat Sentence, Sentence Build, Short Answer Q&A, Story Retell, Open Response. *(P1: 2–3 types · P2: all 6)* — All 6 implemented
...
- ⚠️ **STU-10** — Personal progress dashboard: mastery over time, attempt history, readiness estimate with confidence range. *(P2)* — Basic progress, no readiness estimate with confidence
...
- ✅ **STU-15** — Anti-proctoring: right-click disable, copy/paste block, screenshot shortcuts, text selection disabled, visual notice. *(P1)*
- ✅ **STU-16** — Exam resume: in-progress attempt detection, resume button, runner recovery from any interruption. *(P1)*
- ✅ **STU-17** — Student review/rating: 5-star rating, comment, difficulty feedback after every exam. *(P1)*
- ✅ **STU-18** — Profile editing: full name, roll number, branch, year, L1 language. *(P1)*
- ✅ **STU-19** — Notification preferences: practice reminders, exam deadline alerts. *(P1)*

## 5. Core Assessment Engine (Own-Build)

**Layer 1 — Hearing**
- ✅ **ENG-01** — Speech-to-text transcription with word-level timestamps. *(P1)* — faster-whisper
- ✅ **ENG-02** — Voice activity detection → pause/response-latency features. *(P1)* — Tier 0 (energy) + Tier 1 (Silero)
- ✅ **ENG-03** — Forced alignment (word/phoneme timestamps). *(P1)* — wav2vec2 CTC for pronunciation

**Layer 2 — Speech Quality**
- ✅ **ENG-04** — Phoneme-level pronunciation accuracy scoring. *(P1)* — wav2vec2 GOP (character-level)
- ⚠️ **ENG-05** — Fluency/prosody scoring — interpretable, feature-based model. *(P1)* — Tier 0 feature-based (rate, pauses, phonation). No pitch/prosody model (Tier 1 missing)
- ✅ **ENG-06** — Disfluency detection (fillers, repetitions, self-corrections). *(P1)*
- ❌ **ENG-07** — Intelligibility scoring, trained on Indian-L1 human ratings — **the core differentiator.** *(P3, gated on ENG-09 data)* — Contract exists, no model
- ❌ **ENG-08** — L1/accent classification for routed feedback and difficulty stats. *(P2)* — Contract exists
...
**Layer 3 — Content**
- ⚠️ **ENG-10** — Grammar error detection and typing. *(P1)* — Rule-based (40 regexes), not ML. Indian English excluded
- ✅ **ENG-11** — Content relevance/recall scoring for retell tasks (rubric-constrained, never sole LLM judgment). *(P1)* — Word-overlap coverage

**Layer 4 — Intelligence**
- ❌ **ENG-12** — Item calibration (IRT: difficulty + discrimination, L1-split). *(P2)* — IRT module exists but not wired
- ❌ **ENG-13** — Per-skill mastery tracking (Bayesian Knowledge Tracing). *(P2)* — SkillMastery document exists, no BKT engine
- ❌ **ENG-14** — Adaptive item selection from calibrated item information. *(P3)* — Random sampling with filters only
...
- 🏗️ **ENG-20** — Side-by-side shadow-mode comparison before promotion. *(P2)* — Contract exists
...
- ❌ **NOTIF-01** — Pluggable channel providers (email, SMS, WhatsApp, push) — same abstraction pattern as the engine. *(P1)* — Contract exists, no provider
- ❌ **NOTIF-02** — Per-tenant, per-notification-type channel configuration. *(P1)*
- ❌ **NOTIF-03** — Templated, localizable notification content. *(P2)*
- ❌ **NOTIF-04** — Stored, respected channel opt-in/opt-out as part of consent record. *(P1)* — Consent record exists, channels not implemented

## 8. Payments & Billing

- ❌ **BILL-01** — Pluggable payment gateway — Razorpay Day-1, PayU/Stripe-ready contract. *(P1)* — Contract exists, no provider
...
- ❌ **BILL-04** — Automated GST-compliant invoicing. *(P1)* — Invoice generation exists, auto-GST not implemented
...
- ✅ **BILL-08** — No raw payment credential storage — gateway-handled PCI compliance only. *(P1)* — No gateway yet, but by design

## 9. Reporting & Analytics

- ✅ **RPT-01** — Three reporting tiers: platform / tenant / individual, role-gated. *(P1)*
- ⚠️ **RPT-02** — Exportable, schedulable reports (CSV/PDF). *(P2)* — CSV export implemented, PDF = browser print, scheduling not implemented
...
- ✅ **INT-04** — Consent + audit logging on any data leaving the platform. *(P1)* — Audit logging implemented
...
|---|---|---|
| **P1 — Core MVP** | Tenant CRUD + basic billing · onboarding, cohorts, assignment · student app on **all 6 task types** · own-build ASR/VAD/feature-based fluency · Provider Abstraction Layer live from day one · DPDP consent flow · hard tenant isolation | ✅ **Complete** |
| **P2 — Depth** | IRT + BKT · tenant branding/SSO · cross-tenant analytics · shadow-mode/canary tooling · multi-channel notifications · Intelligibility model (ENG-07) | ❌ **Not Started** (IRT/BKT/P3 items missing) |
| **P3 — Moat** | Intelligibility model (ENG-07) · adaptive item selection · calibrated outcome prediction (ENG-15) · third-party provider adapters · provider marketplace · mobile app | ❌ **Not Started** |
