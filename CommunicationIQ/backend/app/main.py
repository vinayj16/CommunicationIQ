...
FastAPI + SQLAlchemy (async) + PostgreSQL, schema-per-tenant. Control plane in
``public``; each institution in its own ``tenant_<slug>`` schema, resolved from
the authenticated session and nowhere else.
"""
import asyncio
import logging
