...
from app.models.tenant import QuizItem, ReadingAttempt, ReadingPassage, SkillMastery
...
async def passages(principal: Principal,
                   session: TenantSession) -> list[ReadingPassageOut]:
    rows = list((await session.execute(
        select(ReadingPassage).where(ReadingPassage.status == "published")
        .order_by(ReadingPassage.difficulty)
    )).scalars().all())

    counts = dict((await session.execute(
        select(QuizItem.passage_id, func.count())
        .where(QuizItem.category == "reading_comprehension",
               QuizItem.status == "published")
        .group_by(QuizItem.passage_id)
    )).all())

    best = dict((await session.execute(
        select(ReadingAttempt.passage_id, func.max(ReadingAttempt.score))
        .where(ReadingAttempt.user_id == principal.user_id,
               ReadingAttempt.score.is_not(None))
        .group_by(ReadingAttempt.passage_id)
    )).all())
...
async def start(passage_id: str, principal: Principal,
                session: TenantSession) -> ReadingStart:
    ...
    trusted.
    """
    passage = await session.get(ReadingPassage, passage_id)
    ...
    attempt = ReadingAttempt(user_id=principal.user_id, passage_id=passage_id,
                             total=total)
    session.add(attempt)
...
async def questions(attempt_id: str, principal: Principal,
                    session: TenantSession) -> list[ReadingQuestionOut]:
...
async def submit(attempt_id: str, body: ReadingSubmission,
                 ...
                 platform: PlatformSession) -> ReadingResult:
    attempt = await session.get(ReadingAttempt, attempt_id)
    if attempt is None or attempt.user_id != principal.user_id:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "No such attempt")
    if attempt.completed_at is not None:
    ...
    passage = await session.get(ReadingPassage, attempt.passage_id)
    ...
    await _update_reading_mastery(session, principal.user_id, score)
    await session.commit()
...
async def _update_reading_mastery(session, user_id: str, score: float) -> None:
    ...
    row = (await session.execute(
        select(SkillMastery).where(SkillMastery.user_id == user_id,
                                   SkillMastery.skill == "vocabulary")
    )).scalars().first()

    observed = max(0.0, min(1.0, (score - SCALE_MIN) / (SCALE_MAX - SCALE_MIN)))
    if row is None:
        session.add(SkillMastery(user_id=user_id, skill="vocabulary",
