...
from app.models.tenant import Badge, EarnedBadge, SkillMastery, XPLedger
...
async def state(principal: Principal, session: TenantSession,
                platform: PlatformSession) -> GameState:
    config = await engine.config_for(platform, principal.tenant_id)
    user_id = principal.user_id

    await engine.grant_monthly_freezes(session, config, user_id)
    quest = await engine.daily_quest(session, user_id)
    ...
    await engine.qualify_today(session, config, user_id)
    season = await engine.season_for(session, user_id)
    streak = await engine.streak_state(session, user_id)
    await session.commit()

    xp = await engine.total_xp(session, user_id)
    ...
    earned = list((await session.execute(
        select(EarnedBadge).where(EarnedBadge.user_id == user_id)
    )).scalars().all())
    definitions = {b.id: b for b in (await session.execute(select(Badge))).scalars().all()}
...
async def ledger(principal: Principal, session: TenantSession,
    ...
    rows = list((await session.execute(
        select(XPLedger).where(XPLedger.user_id == principal.user_id)
        .order_by(XPLedger.at.desc()).limit(min(limit, 200))
    )).scalars().all())
    return [
        LedgerEntry(
...
async def badges(principal: Principal, session: TenantSession) -> list[BadgeOut]:
    """Every badge, earned or not — the unearned ones say what to aim at."""
    definitions = await engine.ensure_badges(session)
    await session.commit()

    earned = {e.badge_id: e for e in (await session.execute(
        select(EarnedBadge).where(EarnedBadge.user_id == principal.user_id)
    )).scalars().all()}
