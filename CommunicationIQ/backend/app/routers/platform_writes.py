...
from app.deps import PlatformSession, Principal, require_platform
from app.engine.contracts import Capability
from app.engine.registry import clear_provider_cache
from app.models.platform import (TENANT_TYPES, TENANT_TYPE_KEYS,
                                 GamificationConfig, Invoice, Plan,
...
async def configure_capability(capability: str, body: CapabilityConfigRequest,
    ...
    async def resolve(provider_id: str | None, label: str) -> str | None:
        if not provider_id:
            return None
        row = await session.get(ProviderRegistry, provider_id)
    ...
    existing = (await session.execute(
        select(ProviderConfig).where(ProviderConfig.capability == cap.value,
                                     ProviderConfig.tenant_id.is_(body.tenant_id))
        if body.tenant_id else
        select(ProviderConfig).where(ProviderConfig.capability == cap.value,
...
async def register_provider(body: ProviderRegisterRequest, principal: Principal,
                            session: PlatformSession) -> ProviderOut:
...
async def set_provider_active(provider_id: str, active: bool, principal: Principal,
                              session: PlatformSession) -> dict:
    row = await session.get(ProviderRegistry, provider_id)
    if row is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Provider not found")
    ...
    if not active:
        serving = (await session.execute(
            select(func.count()).select_from(ProviderConfig)
            .where(ProviderConfig.primary_provider_id == provider_id))).scalar_one()
        if serving:
            raise HTTPException(
                status.HTTP_409_CONFLICT,
...
async def _tenant_out(session, tenant: Tenant, *, seats_used: int | None = None
                      ) -> TenantOut:
    plan = await session.get(Plan, tenant.plan_id) if tenant.plan_id else None
    ...
    if seats_used is None:
        seats_used = 0
        try:
            async with tenant_sessionmaker(tenant.slug)() as tsession:
                seats_used = int((await tsession.execute(
                    select(func.count()).select_from(User)
                    .where(User.active.is_(True)))).scalar_one())
...
async def set_logo_url(tenant_id: str, body: LogoByUrlRequest,
                       principal: Principal,
                       session: PlatformSession) -> TenantOut:
...
async def create_tenant(body: TenantCreateRequest, principal: Principal,
                        session: PlatformSession) -> TenantOut:
    ...
    if (await session.execute(select(Tenant).where(Tenant.slug == slug))).scalars().first():
    ...
    plan = await session.get(Plan, body.plan_id) if body.plan_id else None
    ...
    async with tenant_sessionmaker(slug)() as tsession:
        tsession.add(User(email=body.admin_email.lower(), full_name=body.admin_name,
                          role="tenant_admin", password_hash=hash_password(password),
                          must_change_password=True))
        await tsession.commit()

    session.add(TenantUserDirectory(email=body.admin_email.lower(),
                                    tenant_id=tenant.id, tenant_slug=slug))
    await session.commit()
    ...
    return await _tenant_out(session, tenant, seats_used=1)


@router.patch("/tenants/{tenant_id}", response_model=TenantOut)
async def update_tenant(tenant_id: str, body: TenantUpdateRequest,
                        principal: Principal, session: PlatformSession) -> TenantOut:
    tenant = await session.get(Tenant, tenant_id)
    if tenant is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Tenant not found")
...
async def create_plan(body: PlanRequest, principal: Principal,
                      session: PlatformSession) -> PlanOut:
    ...
    latest = (await session.execute(
        select(func.max(Plan.version)).where(Plan.code == body.code)
    )).scalar_one()

    plan = Plan(code=body.code, name=body.name, version=(latest or 0) + 1,
...
async def update_plan(plan_id: str, body: PlanUpdateRequest,
    ...
    if body.active is False:
        ...
        in_use = int((await session.execute(
            select(func.count()).select_from(Tenant)
            .where(Tenant.plan_id == plan.id))).scalar_one())
        if in_use:
            raise HTTPException(
                status.HTTP_409_CONFLICT,
...
async def new_plan_version(plan_id: str, body: PlanRequest,
    ...
    """
    source = await session.get(Plan, plan_id)
    if source is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Plan not found")

    highest = int((await session.execute(
        select(func.max(Plan.version)).where(Plan.code == source.code))
    ).scalar_one() or source.version)

    plan = Plan(
        code=source.code, name=body.name, version=highest + 1,
        billing_model=body.billing_model, currency=body.currency,
        price_per_seat=body.price_per_seat, price_flat=body.price_flat,
        attempt_allowance=body.attempt_allowance,
        features=body.features or {}, active=True,
...
@router.post("/tenants/{tenant_id}/invoice", response_model=InvoiceOut,
             status_code=status.HTTP_201_CREATED)
async def issue_invoice(tenant_id: str, principal: Principal,
                        session: PlatformSession) -> InvoiceOut:
    ...
    async with tenant_sessionmaker(tenant.slug)() as tsession:
        seats = int((await tsession.execute(
            select(func.count()).select_from(User).where(User.active.is_(True))
        )).scalar_one())
    ...
    count = int((await session.execute(
        select(func.count()).select_from(Invoice))).scalar_one())
...
async def invoices(session: PlatformSession) -> list[InvoiceOut]:
    rows = list((await session.execute(
        select(Invoice).order_by(Invoice.created_at.desc()).limit(100)
    )).scalars().all())
    names = dict((await session.execute(select(Tenant.id, Tenant.name))).all())
    return [
        InvoiceOut(
...
async def update_gamification(body: GamificationConfigRequest, principal: Principal,
                              session: PlatformSession) -> GamificationConfigOut:
    ...
    row = (await session.execute(
        select(GamificationConfig).where(GamificationConfig.tenant_id == body.tenant_id)
        if body.tenant_id else
