...
(capabilities, providers, latency, audit). Institution schemas are not
...
from app.deps import PlatformSession, require_platform
...
async def overview(session: PlatformSession) -> PlatformOverview:
    tenants = (await session.execute(select(Tenant))).scalars().all()
    plans = (await session.execute(select(func.count()).select_from(Plan))).scalar_one()
    providers = (await session.execute(
        select(func.count()).select_from(ProviderRegistry)
    )).scalar_one()
    configured = (await session.execute(
        select(func.count(func.distinct(ProviderConfig.capability)))
    )).scalar_one()
    week_ago = datetime.now(timezone.utc) - timedelta(days=7)
    audit = (await session.execute(
        select(func.count()).select_from(AuditLog).where(AuditLog.at >= week_ago)
    )).scalar_one()
...
async def tenants(session: PlatformSession) -> list[TenantOut]:
    ...
    # fetched it.
    return [await _tenant_out(session, t) for t in rows]


@router.get("/plans", response_model=list[PlanOut])
async def plans(session: PlatformSession) -> list[PlanOut]:
    rows = (await session.execute(
        select(Plan).order_by(Plan.code, Plan.version.desc())
    )).scalars().all()
    return [
        PlanOut(
...
async def capabilities(session: PlatformSession) -> list[CapabilityOut]:
    ...
    registry = (await session.execute(
        select(ProviderRegistry).order_by(ProviderRegistry.capability, ProviderRegistry.tier)
    )).scalars().all()
    configs = {c.capability: c for c in (await session.execute(
        select(ProviderConfig).where(ProviderConfig.tenant_id.is_(None))
    )).scalars().all()}

    day_ago = datetime.now(timezone.utc) - timedelta(days=1)
    stats = {
        pid: (calls, errors, latency)
        for pid, calls, errors, latency in (await session.execute(
            select(ProviderCall.provider_id, func.count(),
                   func.count().filter(ProviderCall.ok.is_(False)),
                   func.avg(ProviderCall.latency_ms))
            .where(ProviderCall.at >= day_ago)
            .group_by(ProviderCall.provider_id)
        )).all()
    }
...
async def audit(session: PlatformSession, limit: int = 100) -> list[AuditOut]:
    rows = (await session.execute(
        select(AuditLog).order_by(AuditLog.at.desc()).limit(min(limit, 500))
    )).scalars().all()
    return [
        AuditOut(
...
async def narration_metrics(session: PlatformSession) -> dict:
...
async def gamification(session: PlatformSession,
                       tenant_id: str | None = None) -> GamificationConfigOut:
