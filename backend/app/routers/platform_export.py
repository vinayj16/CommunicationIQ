"""Bulk export of one institution's reports â€” the deliberate exception.
...
because an operator with database credentials already *has* the data â€” the
...
* **The slug still never comes from the caller.** The caller names a tenant
  *id*; the slug is read from the control-plane registry row. TEN-12's
  invariant â€” no code path accepts a database name from outside â€” survives.
...
* **Every tenant status is exportable.** drop_tenant_schema's contract says
  offboarding completes "the contracted data export first" â€” this endpoint
...
def _disarm(value):
    ...
    Strings only â€” numbers arrive as numbers, so a negative score is never
...
Files
-----
students.csv        Everyone who sits assessments â€” students and invited
...
attempts.csv        One row per test attempt, with the overall score where
                    ...
                    report_measures.csv.
report_measures.csv One row per measurement â€” the long-format contents of
...
Reading these files
-------------------
Every score is UNCALIBRATED unless the calibration state says otherwise â€”
...
async def export_tenant_reports(tenant_id: str, principal: Principal) -> HttpResponse:
    ...
    # A registry row whose institution database does not exist â€” never
    # provisioned, or already dropped during offboarding â€” must answer a
    ...
    if not await tenant_schema_exists(tenant.slug):
        raise HTTPException(
            ...
            f"The schema for '{tenant.slug}' does not exist â€” the tenant is "
...
async def _write_reports(models: SimpleNamespace, zf: zipfile.ZipFile,
    ...
    Held in memory across the whole export: the roster (id â†’ email, name),
    the profile names, and one small tuple per attempt â€” its owner and
