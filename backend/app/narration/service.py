...
Concurrency safety in MongoDB uses findOneAndUpdate with sort and limit
...
async def ai_consent_granted(user_id: str) -> bool:
    """The student's latest ai_explanation decision. Absent = not granted."""
    row = await ConsentRecord.find(
        ConsentRecord.user_id == user_id,
        ConsentRecord.scope == "ai_explanation"
    ).sort(-ConsentRecord.at).first_or_none()
    return bool(row and row.granted)


async def ensure_row(attempt: Attempt) -> AttemptNarration | None:
    ...
    existing = await AttemptNarration.find(
        AttemptNarration.attempt_id == attempt.id
    ).first_or_none()
    if existing is not None:
        return existing

    user = await User.get(attempt.user_id)
    if user is None or not await ai_consent_granted(user.id):
        return None
...
async def claim_due(slug: str, *, batch: int) -> list[str]:
    ...
    dead lease. Uses findOneAndUpdate for atomic claim.
    """
    now = _now()
    lease = now + timedelta(seconds=settings.narration_lease_seconds)
    claimed: list[str] = []

    # Find due jobs - get candidates first
    filter_query = {
        "$or": [
            {"status": "pending"},
    ...
    candidates = await AttemptNarration.find(filter_query).sort(+AttemptNarration.created_at).limit(batch).to_list()

    for row in candidates:
        # Try to atomically claim each one
        result = await AttemptNarration.find_one(
            AttemptNarration.id == row.id,
            AttemptNarration.status == row.status
        ).update(
            {
                "$set": {
                    "status": "processing",
                    "lease_until": lease,
                    "next_retry_at": None,
                    "attempt_count": AttemptNarration.attempt_count + 1,
                }
            }
        )
...
async def claim_one(slug: str, narration_id: str) -> bool:
    ...
    row = await AttemptNarration.find_one(AttemptNarration.id == narration_id)
    if row is None:
        return False
    ...
    result = await AttemptNarration.find_one(AttemptNarration.id == narration_id).update(
        {
            "$set": {
                "status": "processing",
                "lease_until": lease,
...
async def generate_one(slug: str, narration_id: str) -> str:
    ...
    try:
        attempt = await Attempt.get(row.attempt_id)
        if attempt is None:
...
async def _record_failure(row: AttemptNarration, err: NarratorError) -> str:
